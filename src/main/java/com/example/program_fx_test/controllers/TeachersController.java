...
public class TeachersController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            studentUsers = repository.getAllUsers().stream().filter(user -> user.getRole() == Roles.TEACHER).toList();
        ...
        addButton.setOnAction(event -> {
            sceneCreator.newScene("addTeacher.fxml");
        });

        Roles currentUserRole = Application.getCurrentUserRole();
        if (currentUserRole.equals(Roles.DEKAN)) {
        ...
        reload.setOnAction(e -> {
            ...
            try {
                studentsReloaded = repository.getAllUsers().stream().filter(user -> user.getRole() == Roles.TEACHER).toList();
        ...
        deleteButton.setOnAction(e-> {
            ...
            try {
                ...
                List<User> studentsReloaded;
                try {
                    studentsReloaded = repository.getAllUsers().stream().filter(u -> u.getRole() == Roles.TEACHER).toList();
                } catch (SQLException | ClassNotFoundException e2) {
                    throw new RuntimeException(e2);
                }
                tableStudents.getItems().setAll(studentsReloaded);
